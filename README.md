...
## gbj_appbase()
#### Description
Destructor `~gbj_appbase()` just releases the two-wire bus.
Constructor `gbj_appbase()` creates the class instance object and sets some bus parameters that are usually constant in a sketch. On the other hand, constructor parameters can be changed individually later in a sketch, if it is needed to change them dynamically.
...
#### Syntax
    gbj_appbase(uint32_t clockSpeed, uint8_t pinSDA, uint8_t pinSCL);
...
## begin()
#### Description
The method checks the microcontroller's pins defined in the [constructor](gbj_appbase) for software defined two-wire bus platforms and initiates the two-wire bus. So that, it should be called at the beginning of every corresponding method of an inherited class.
...
{
  if (gbj_appbase::begin()) return getLastResult();
  ...
}
```
...
#### Example
```cpp
gbj_appbase Object = gbj_appbase();
if (Object.registerAddress(newAddress) == gbj_appbase::SUCCESS)
{
...
if (Object.getLastResult() == gbj_appbase::SUCCESS)
{
  Serial.println("Success");
}
else
{
  ...
  {
    case gbj_appbase::ERROR_ADDRESS:
      Serial.println("Bad address");
      break;

    case gbj_appbase::ERROR_NACK_OTHER:
